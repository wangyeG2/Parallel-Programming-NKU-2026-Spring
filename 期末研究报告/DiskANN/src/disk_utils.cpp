...
namespace diskann
{
void generate_bfs_reorder_map(const std::string& mem_index_file, size_t num_points, 
                              std::vector<uint32_t>& reverse_map) {
    // 1. 读取内存索引文件头部获取 start_node 和图结构
    std::ifstream vamana_reader(mem_index_file, std::ios::binary);
    uint64_t expected_file_size;
    uint32_t width_u32, start_node;
    uint64_t frozen_num;
    vamana_reader.read((char*)&expected_file_size, sizeof(uint64_t));
    vamana_reader.read((char*)&width_u32, sizeof(uint32_t));
    vamana_reader.read((char*)&start_node, sizeof(uint32_t));
    vamana_reader.read((char*)&frozen_num, sizeof(uint64_t));

    // 2. 预先读取图的邻接表到内存中 (假设图不大，若过大可分块处理)
    std::vector<std::vector<uint32_t>> graph(num_points);
    for (uint32_t i = 0; i < num_points; i++) {
        uint32_t nnbrs;
        vamana_reader.read((char*)&nnbrs, sizeof(uint32_t));
        graph[i].resize(nnbrs);
        vamana_reader.read((char*)graph[i].data(), nnbrs * sizeof(uint32_t));
    }
    vamana_reader.close();

    // 3. 从 start_node 开始 BFS
    reverse_map.clear();
    reverse_map.reserve(num_points);
    ...
    while (!q.empty()) {
        ...
        reverse_map.push_back(u); // 记录物理顺序
        
        for (uint32_t v : graph[u]) {
            if (v < num_points && !visited[v]) { // 忽略 frozen point 越界
                visited[v] = true;
                q.push(v);
            }
        }
...
void create_disk_layout(const std::string base_file, const std::string mem_index_file,
                        ...
                        const std::vector<uint32_t>& reverse_map)
{
    uint32_t npts, ndims;
    ...
    // 因为需要随机读取，这里改用标准的 ifstream 并配合 seekg
    std::ifstream base_reader_std(base_file, std::ios::binary);
    if (!base_reader_std.is_open()) {
        throw diskann::ANNException("Cannot open base_file", -1);
    }
    base_reader_std.read((char *)&npts, sizeof(uint32_t));
    base_reader_std.read((char *)&ndims, sizeof(uint32_t));
    ...
    // 预计算 mem_index_file 中每个 node 的偏移量，以便支持随机读取
    std::vector<std::streampos> node_offsets(npts_64);
    // 跳过文件头
    vamana_reader.seekg(sizeof(uint64_t) + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint64_t));
    for (uint64_t i = 0; i < npts_64; i++) {
        node_offsets[i] = vamana_reader.tellg();
        uint32_t tmp_nnbrs;
        vamana_reader.read((char*)&tmp_nnbrs, sizeof(uint32_t));
        vamana_reader.seekg(tmp_nnbrs * sizeof(uint32_t), std::ios::cur);
    }
    ...
    { // Write multiple nodes per sector
        for (uint64_t sector = 0; sector < n_sectors; sector++)
        {
                 ...
                 sector_node_id++)
            {
                ...
                // 核心改变：获取当前物理位置对应的原始 node_id
                uint64_t original_id = reverse_map.empty() ? cur_node_id : reverse_map[cur_node_id];

                // 从 vamana_reader 随机读取图邻居
                vamana_reader.clear();
                vamana_reader.seekg(node_offsets[original_id]);
                vamana_reader.read((char *)&nnbrs, sizeof(uint32_t));
                ...
                // 从 base_reader_std 随机读取向量数据
                base_reader_std.clear();
                base_reader_std.seekg(sizeof(uint32_t) * 2 + original_id * sizeof(T) * ndims_64, std::ios::beg);
                base_reader_std.read((char *)cur_node_coords.get(), sizeof(T) * ndims_64);
    ...
    { // Write multi-sector nodes
        ...
        for (uint64_t i = 0; i < npts_64; i++)
        {
            ...
            // 核心改变：获取当前物理位置对应的原始 node_id
            uint64_t original_id = reverse_map.empty() ? i : reverse_map[i];
    ...
    if (append_reorder_data)
    {
        ...
        for (uint64_t sector = 0; sector < n_reorder_sectors; sector++)
        {
            ...
            for (uint64_t sector_node_id = 0; sector_node_id < n_data_nodes_per_sector && cur_reorder_id < npts_64;
                 sector_node_id++)
            {
                memset(vec_buf.get(), 0, vec_len);
                
                // 根据 reverse_map 随机读取对应的 reorder data
                uint64_t original_id = reverse_map.empty() ? cur_reorder_id : reverse_map[cur_reorder_id];
                reorder_data_reader.clear();
                reorder_data_reader.seekg(8 + original_id * vec_len, std::ios::beg);
    ...
    // 把 reverse_map 保存到磁盘，供搜索时加载
    if (!reverse_map.empty()) {
        std::vector<uint32_t> forward_map(reverse_map.size());
        for (uint32_t phys_id = 0; phys_id < reverse_map.size(); phys_id++) {
            uint32_t node_id = reverse_map[phys_id];
            forward_map[node_id] = phys_id;
        }
        
        std::string map_file = output_file + "_reorder_map.bin";
        // 创建一个非 const 副本以满足 save_bin 函数的参数要求
        std::vector<uint32_t> non_const_map = forward_map;
        diskann::save_bin<uint32_t>(map_file, non_const_map.data(), non_const_map.size(), 1, 0);
        diskann::cout << "Reorder map (inverted to forward_map) saved to " << map_file << std::endl;
    }
...
// This code should only be available in the "build" DLL.
    ...
    // 生成 BFS 重排映射表
    std::vector<uint32_t> reverse_map;
    generate_bfs_reorder_map(mem_index_path, points_num, reverse_map);
    diskann::cout << "Generated BFS reorder map of size " << reverse_map.size() << std::endl;

    if (!use_disk_pq)
    {
        diskann::create_disk_layout<T>(data_file_to_use.c_str(), mem_index_path, disk_index_path, "", reverse_map);
    }
    else
    {
        if (!reorder_data)
            diskann::create_disk_layout<uint8_t>(disk_pq_compressed_vectors_path, mem_index_path, disk_index_path, "", reverse_map);
        else
            diskann::create_disk_layout<uint8_t>(disk_pq_compressed_vectors_path, mem_index_path, disk_index_path,
                                                 data_file_to_use.c_str(), reverse_map);
    }
...
template DISKANN_DLLEXPORT void create_disk_layout<int8_t>(const std::string base_file, const std::string mem_index_file, const std::string output_file, const std::string reorder_data_file, const std::vector<uint32_t>& reverse_map);
template DISKANN_DLLEXPORT void create_disk_layout<uint8_t>(const std::string base_file, const std::string mem_index_file, const std::string output_file, const std::string reorder_data_file, const std::vector<uint32_t>& reverse_map);
template DISKANN_DLLEXPORT void create_disk_layout<float>(const std::string base_file, const std::string mem_index_file, const std::string output_file, const std::string reorder_data_file, const std::vector<uint32_t>& reverse_map);
