...
template <typename T, typename LabelT>inline uint64_t PQFlashIndex<T, LabelT>::get_node_sector(uint64_t node_id) {
    // 核心修改：如果发生了重排，将逻辑 node_id 转为物理 phys_id
    uint64_t phys_id = _is_reordered ? _reorder_map[node_id] : node_id;
    return 1 + (_nnodes_per_sector > 0 ? phys_id / _nnodes_per_sector : phys_id * DIV_ROUND_UP(_max_node_len, defaults::SECTOR_LEN));
}

template <typename T, typename LabelT>
inline char *PQFlashIndex<T, LabelT>::offset_to_node(char *sector_buf, uint64_t node_id) {
    // 核心修改：同理使用 phys_id 计算扇区内的偏移
    uint64_t phys_id = _is_reordered ? _reorder_map[node_id] : node_id;
    return sector_buf + (_nnodes_per_sector == 0 ? 0 : (phys_id % _nnodes_per_sector) * _max_node_len);
}
...
// read index metadata
    ...
    std::string reorder_map_file = std::string(_disk_index_file) + "_reorder_map.bin";
    if (file_exists(reorder_map_file)) {
        uint64_t dummy_nrows, dummy_ncols;
        diskann::load_bin<uint32_t>(reorder_map_file, _reorder_map, dummy_nrows, dummy_ncols);
        // 使用总元素个数校验，防止行列读取不一致
        if (dummy_nrows * dummy_ncols == this->_num_points) {
            _is_reordered = true;
            diskann::cout << "Loaded BFS reorder map. SSD layout is reordered." << std::endl;
        } else {
            diskann::cout << "Warning: Reorder map size mismatch. Expected: " << this->_num_points 
                          << ", Got: " << dummy_nrows * dummy_ncols << ". Ignoring." << std::endl;
            delete[] _reorder_map;
            _reorder_map = nullptr;
        }
    }
