#ifndef IVF_HNSW_SERIAL_H
#define IVF_HNSW_SERIAL_H
...
public:
    ...
    IVF_HNSW_Serial_Index(size_t dimension, int num_clusters, int m, int ef_constr)
    ...
    void build_and_save(float* base, size_t n, const std::string& index_path) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            for (int c = 0; c < nlist; ++c) {
                ...
                if (d < min_dist) { min_dist = d; best_cluster = c; }
            }
            clusters_data[best_cluster].insert(clusters_data[best_cluster].end(), base + i * dim, base + (i + 1) * dim);
            clusters_ids[best_cluster].push_back(i);
        }

        std::cout << "Building HNSW for each cluster (Serial)..." << std::endl;
        hnsw_indices.resize(nlist, nullptr);
        for (int c = 0; c < nlist; ++c) {
            size_t cluster_size = clusters_ids[c].size();
            if (cluster_size == 0) continue;
            hnswlib::HierarchicalNSW<float>* appr_alg = new hnswlib::HierarchicalNSW<float>(ipspace, cluster_size, M, ef_construction);
    ...
    std::priority_queue<std::pair<float, uint32_t>> search(const float* query, size_t k, int nprobe, int ef_search) {
